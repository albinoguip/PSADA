...
class DYN_Functions(MainWindow):
    ...
    def DYNAMIC_search_button_FUNCTION(self): 
    ...
    def DYNAMIC_read_file_button_FUNCTION(self):

        PATH    = self.ui.DYNAMIC_qline_files.text()
        POUT    = '/'.join(self.ui.DYNAMIC_qline_files.text().split('/')[:-2]) + '/OUT/'
        ...
        self.ui.DYNAMIC_vars_qline.setText(PATH.replace('OUT/', '') + 'vars.csv')    
    ...
    def DYNAMIC_vars_search_FUNCTION(self): 
    ...
    def DYNAMIC_read_vars_button_FUNCTION(self): 

        file_vars = self.ui.DYNAMIC_vars_qline.text()
    ...
    def DYNAMIC_plot_button_FUNCTION(self):

        # if len(self.DYNAMIC_plot) == 0:
        ...
        r_var = self.ui.DYNAMIC_round.currentText()
        s_var = self.ui.DYNAMIC_stats.currentText()

        self.DYNAMIC_plot = {'x' : x_var,
                                'y' : y_var,
                                'c' : c_var,
        ...
        data = pd.read_csv(self.ui.DYNAMIC_vars_qline.text())

        guip = GUI_Plotter(data, self.DYNAMIC_plot, self.ui)
        guip.update()


    def DYNAMIC_button_apply_filter_FUNCTION(self):

        f_var = self.ui.DYNAMIC_combo_variable.currentText()
        i_var = self.ui.DYNAMIC_combo_sinal.currentText()
        t_var = self.ui.DYNAMIC_label_value.text()
        ...
        if self.DYNAMIC_plot['filter'] is None or self.DYNAMIC_plot['filter'] == '':
            self.DYNAMIC_plot['filter'] = [(f_var, i_var, t_var)]
        
        else:
            self.DYNAMIC_plot['filter'].append((f_var, i_var, t_var))       
    ...
    def DYNAMIC_add_section_button_FUNCTION(self):

        

        self.DYN_json_to_plot[self.ui.DYNAMIC_add_section.text()] = {}

        self.DYN_sections.append(self.ui.DYNAMIC_add_section.text())

        for i in range(len(self.DYN_sections)-1):
    ...
    def DYNAMIC_add_plot_button_FUNCTION(self):

        self.DYN_json_to_plot[self.ui.DYNAMIC_sections_ava.currentText()][self.ui.DYNAMIC_add_plot.text()] = []
    ...
    def DYNAMIC_select_section_FUNCTION(self):
    ...
    def DYNAMIC_add_to_json_button_FUNCTION(self):
        
        page    = self.ui.DYNAMIC_select_plot.currentText()
        section = self.ui.DYNAMIC_select_section.currentText()

        self.DYN_json_to_plot[section][page].append(self.DYNAMIC_plot)


    def DYNAMIC_page_to_plot_button_FUNCTION(self):

        page    = self.ui.DYNAMIC_select_plot.currentText()
        section = self.ui.DYNAMIC_select_section.currentText()

        self.DYNAMIC_plot = self.DYN_json_to_plot[section][page][0]


        temp_filter = []
        if self.DYNAMIC_plot['filter'] is not None and self.DYNAMIC_plot['filter'] != '':

            for f_var, i_var, t_var in self.DYNAMIC_plot['filter']:
            ...
            self.DYNAMIC_plot['filter'] = temp_filter   
    ...
    def DYNAMIC_save_json_button_FUNCTION(self):
        
        path = self.ui.DYNAMIC_vars_qline.text().split('/')
        path = '/'.join(path[:-1]) + '/plot.json'


        with open(path, 'w') as fp:
            json.dump(self.DYN_json_to_plot, fp)





    def DYNAMIC_search_json_button_FUNCTION(self): 


        self.file_vars, _ = QFileDialog.getOpenFileNames(self, "Open File", "/home", "json (*.json);;All files(*.*)")
        self.file_vars    = self.file_vars[0]

        self.ui.DYNAMIC_qline_json_files.setText(self.file_vars)    


    def DYNAMIC_read_json_button_FUNCTION(self): 
    ...
    def run_DYNAMIC_FUNCTION(self): 
        ...
        self.ui.DYNAMIC_document.load(save_path)


    def actionSave_DYNAMIC_FUNCTION(self): 
    ...
    def actionZoom_In_DYNAMIC_FUNCTION(self):

        factor = self.ui.pdfView_DYNAMIC.zoomFactor() * 2
        self.ui.pdfView_DYNAMIC.setZoomFactor(factor)

    
    def actionZoom_Out_DYNAMIC_FUNCTION(self):

        factor = self.ui.pdfView_DYNAMIC.zoomFactor() / 2
        self.ui.pdfView_DYNAMIC.setZoomFactor(factor)

    def actionPage_down_DYNAMIC_FUNCTION(self):

        nav = self.ui.pdfView_DYNAMIC.pageNavigator()
        nav.jump(nav.currentPage() - 1, QPoint(), nav.currentZoom())

    def actionPage_up_DYNAMIC_FUNCTION(self):
