...
class NET_Functions(MainWindow):
    ...
    def NETWORK_search_button_FUNCTION(self): 

        self.file_data_match = str(QFileDialog.getExistingDirectory(self, "Select Directory")) + '/'

        self.ui.NETWORK_qline_files.setText(self.file_data_match)



    def NETWORK_read_file_button_FUNCTION(self):

        PATH    = self.ui.NETWORK_qline_files.text()
        POUT    = '/'.join(self.ui.NETWORK_qline_files.text().split('/')[:-2]) + '/OUT/'
        ...
        self.ui.NETWORK_vars_qline.setText(PATH.replace('OUT/', '') + 'vars.csv')    
    ...
    def NETWORK_vars_search_FUNCTION(self): 


        self.file_vars, _ = QFileDialog.getOpenFileNames(self, "Open File", "/home", "Excel (*.csv);;All files(*.*)")
        self.file_vars    = self.file_vars[0]

        self.ui.NETWORK_vars_qline.setText(self.file_vars)     




    def NETWORK_read_vars_button_FUNCTION(self): 

        file_vars = self.ui.NETWORK_vars_qline.text()
        ...
        self.ui.NETWORK_x.addItems(self.rst_generic._get_variables())
        self.ui.NETWORK_y.addItems(self.rst_generic._get_variables())
        self.ui.NETWORK_c.addItems(c_options)

        self.ui.NETWORK_plot.addItems(['Scatter', 'Line', 'Histogram'])
        self.ui.NETWORK_round.addItems(r_options)
        self.ui.NETWORK_stats.addItems([None, 'Mean', 'Sum', 'Std'])

        self.ui.NETWORK_combo_variable.addItems(self.rst_generic._get_variables())
        self.ui.NETWORK_combo_sinal.addItems(['>', '>=', '<', '<=', '==', 'List', 'not_NaN', 'NaN'])
        


    def NETWORK_plot_button_FUNCTION(self):

        # if len(self.NETWORK_plot) == 0:
            
        x_var = self.ui.NETWORK_x.currentText()
        y_var = self.ui.NETWORK_y.currentText()
        c_var = self.ui.NETWORK_c.currentText()

        p_var = self.ui.NETWORK_plot.currentText()
        r_var = self.ui.NETWORK_round.currentText()
        s_var = self.ui.NETWORK_stats.currentText()

        self.NETWORK_plot = {'x' : x_var,
                                'y' : y_var,
                                'c' : c_var,
        ...
        data = pd.read_csv(self.ui.NETWORK_vars_qline.text())

        guip = GUI_Plotter(data, self.NETWORK_plot, self.ui)
        guip.update()


    def NETWORK_button_apply_filter_FUNCTION(self):

        f_var = self.ui.NETWORK_combo_variable.currentText()
        i_var = self.ui.NETWORK_combo_sinal.currentText()
        t_var = self.ui.NETWORK_label_value.text()
        ...
        if self.NETWORK_plot['filter'] is None or self.NETWORK_plot['filter'] == '':
            self.NETWORK_plot['filter'] = [(f_var, i_var, t_var)]
        
        else:
            self.NETWORK_plot['filter'].append((f_var, i_var, t_var))       
    ...
    def NETWORK_add_section_button_FUNCTION(self):

        

        self.NET_json_to_plot[self.ui.NETWORK_add_section.text()] = {}

        self.NET_sections.append(self.ui.NETWORK_add_section.text())

        for i in range(len(self.NET_sections)-1):
            self.ui.NETWORK_sections_ava.removeItem(0)
            self.ui.NETWORK_select_section.removeItem(0)

        self.ui.NETWORK_sections_ava.addItems(self.NET_sections)

        self.ui.NETWORK_select_section.addItems(self.NET_sections)

        for i in range(self.ui.NETWORK_select_plot.count()):
            self.ui.NETWORK_select_plot.removeItem(0)

        self.ui.NETWORK_select_plot.addItems(list(self.NET_json_to_plot[self.ui.NETWORK_select_section.currentText()].keys()))


    def NETWORK_add_plot_button_FUNCTION(self):

        self.NET_json_to_plot[self.ui.NETWORK_sections_ava.currentText()][self.ui.NETWORK_add_plot.text()] = []
    ...
    def NETWORK_select_section_FUNCTION(self):
    ...
    def NETWORK_add_to_json_button_FUNCTION(self):
        
        page    = self.ui.NETWORK_select_plot.currentText()
        section = self.ui.NETWORK_select_section.currentText()

        self.NET_json_to_plot[section][page].append(self.NETWORK_plot)


    def NETWORK_page_to_plot_button_FUNCTION(self):

        page    = self.ui.NETWORK_select_plot.currentText()
        section = self.ui.NETWORK_select_section.currentText()

        self.NETWORK_plot = self.NET_json_to_plot[section][page][0]


        temp_filter = []
        if self.NETWORK_plot['filter'] is not None and self.NETWORK_plot['filter'] != '':

            for f_var, i_var, t_var in self.NETWORK_plot['filter']:
            ...
            self.NETWORK_plot['filter'] = temp_filter   
    ...
    def NETWORK_save_json_button_FUNCTION(self):
        
        path = self.ui.NETWORK_vars_qline.text().split('/')
        path = '/'.join(path[:-1]) + '/plot.json'


        with open(path, 'w') as fp:
            json.dump(self.NET_json_to_plot, fp)





    def NETWORK_search_json_button_FUNCTION(self): 


        self.file_vars, _ = QFileDialog.getOpenFileNames(self, "Open File", "/home", "json (*.json);;All files(*.*)")
        self.file_vars    = self.file_vars[0]

        self.ui.NETWORK_qline_json_files.setText(self.file_vars)    


    def NETWORK_read_json_button_FUNCTION(self): 

        path = self.ui.NETWORK_qline_json_files.text()

        with open(path) as f:
            data = json.load(f)

        self.NET_json_to_plot = data
        self.NET_sections     = list(self.NET_json_to_plot.keys())


        for i in range(self.ui.NETWORK_sections_ava.count()):
            self.ui.NETWORK_sections_ava.removeItem(0)
            self.ui.NETWORK_select_section.removeItem(0)
    ...
    def run_NETWORK_FUNCTION(self): 
        
        base_path = self.ui.NETWORK_qline_json_files.text().split('/')
        save_path = '/'.join(base_path[:-1]) + '/Report.pdf'

        data_p = self.ui.NETWORK_vars_qline.text()
        path   = self.ui.NETWORK_qline_json_files.text()
        ...
        self.ui.NETWORK_document.load(save_path)


    def actionSave_NETWORK_FUNCTION(self): 
    ...
    def actionZoom_In_NETWORK_FUNCTION(self):

        factor = self.ui.pdfView_NETWORK.zoomFactor() * 2
        self.ui.pdfView_NETWORK.setZoomFactor(factor)

    
    def actionZoom_Out_NETWORK_FUNCTION(self):

        factor = self.ui.pdfView_NETWORK.zoomFactor() / 2
        self.ui.pdfView_NETWORK.setZoomFactor(factor)

    def actionPage_down_NETWORK_FUNCTION(self):

        nav = self.ui.pdfView_NETWORK.pageNavigator()
        nav.jump(nav.currentPage() - 1, QPoint(), nav.currentZoom())

    def actionPage_up_NETWORK_FUNCTION(self):
