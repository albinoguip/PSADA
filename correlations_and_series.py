...
class Analise_Fluxos:
    ...
    def __init__(self, df_1, df_2, delimiter_1=';', delimiter_2=';', decimal_1='.', decimal_2=','): # inicializando os df
        ...
        #self.df1['EXP_N'] = self.df1['EXP_NS'] - self.df1['EXP_NEN'] #----- para fazer a aproximação do fluxo N HPPA
        #self.df1['Xingu_Total'] = self.df1['XESTR_MW'] + self.df1['XTRIO_MW'] #----- para obter Xingu Total HPPA
        #self.df1 = self.df1.iloc[::2].reset_index(drop=True) # NAO ESQUECER DE RESETAR OS INDICES PARA QUE A CORRELACAO SEJA FEITA DA FORMA CORRETA
        ...
        #self.df2 = -self.df2
        #self.df2 = self.df2.iloc[::2].reset_index(drop=True) # NAO ESQUECER DE RESETAR OS INDICES PARA QUE A CORRELACAO SEJA FEITA DA FORMA CORRETA
        self._sincronizar_dados()
        #print(self.df1, self.df2)
    ...
    def scatter_plot(self, col_1, col_2):
        ...
        plt.xlabel('Norus')
        plt.ylabel('Real')
        plt.title('Dispersão Xingu Total (MW)')
        plt.show()
    ...
    def time_series(self, col_1, col_2):
        ...
        ax1.plot(self.df1[col_1], label='Norus', c='r') # HPPA
        ax1.plot(self.df2[col_2], label='Real', c='g') # Real
        ax1.axhline(y=0, color='k', linestyle='--', alpha=0.5)
        ax1.set_yticks(np.arange(-5500, 8000, 500))
        ...
        # Adicionando os valores médios de cada coluna dos dfs
        media1 = self.df1[col_1].mean()
        media2 = self.df2[col_2].mean()

        ax1.axhline(y=media1, color='r', linestyle='--', alpha=0.4)
        ax1.axhline(y=media2, color='g', linestyle='--', alpha=0.4)
...
df_1 = 'NORUS fluxo_intercambio_FNS.csv'
df_2 = 'real exp ns.csv' # fluxo real

# df_3 = ... inclusao de mais plots nas series temporais
...
analise.correl('MW', 'EXP_NS')
analise.scatter_plot('MW', 'EXP_NS')
analise.erro('MW', 'EXP_NS')
analise.time_series('MW', 'EXP_NS')